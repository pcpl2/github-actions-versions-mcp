...
- **Application icon and installer artwork** — an upward arrow in GitHub's
  dark/green palette. Embedded in `gha-mcp.exe`, used as the setup icon, and as
  the installer's wizard images. Regenerate with
  `python scripts/generate-assets.py`; the outputs are committed, so building
  never needs Python.
- **Windows file metadata.** `gha-mcp.exe` and the installer now fill in the
  Properties → Details tab: product name, description, company, copyright and
  version. Generated by goversioninfo from `versioninfo.json` during the release
  build, with the version taken from the tag.
